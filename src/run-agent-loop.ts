...
export const runAgentLoop = observe(
  async function runAgentLoop({ question, userId, writer }: AgentLoopOptions) {
    ...
    while (!ctx.shouldStop()) {
      ...
      switch (action.type) {
        case "research":
          ...
          // Step 2: Optionally scrape specified URLs
          if (action.urlsToScrape && action.urlsToScrape.length > 0) {
            console.log(`📄 Scraping ${action.urlsToScrape.length} URL(s)...`);
            const scrapeResults: { url: string; result: string }[] = [];
            for (const url of action.urlsToScrape) {
              try {
                console.log(`→ ${url}`);
        ...
        case "answer": {
          ...
          updateActiveObservation({ output: { status: 'answer_generated', steps: ctx.getStep() } });
    ...
    updateActiveObservation({ output: { status: 'max_steps_reached', steps: ctx.getStep() } });
    return { stream, context: ctx, actions: collectedActions };
  },
  { name: 'Agent Loop' }
);